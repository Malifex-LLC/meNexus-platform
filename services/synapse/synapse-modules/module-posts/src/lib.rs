...
mod server {
    ...
    use synapse_application::events::{
        CreateEventCommand, CreateLocalEventUseCase, CreateRemoteEventCommand,
        CreateRemoteEventUseCase,
    };
    ...
    pub struct PostsDeps {
        pub repo:
            std::sync::Arc<dyn synapse_core::ports::events::event_repository::EventRepository>,
    ...
    #[derive(Deserialize)]
    struct ListPostsForChannelRequest {
        event_type: String,
    }

    #[derive(Serialize)]
    struct ListPostsForChannelResult {
        posts: Vec<Event>,
    }
    ...
    async fn list_posts_for_channel_http(
        ...
        Ok((
            StatusCode::CREATED,
            Json(ListPostsForChannelResult { posts: results }),
        ))
    }
    ...
    async fn create_post(event: &Event) -> Result<Vec<Event>, CoreError> {
